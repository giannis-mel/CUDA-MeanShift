...
// for compile : nvcc nonshared.cu -o nonshared -O3 -lm 
// for run: ./nonshared 0.5

// CUDA kernel to calculate Euclidean distance between two points
__device__ float distanceFunction(float *point1, float *point2) {
    float distance = 0;
...
__device__ float kernelFunction(float *pointYk, float *arrayXi) {
    float s = 1;
...
__device__ float movedDistance(float *moved) {
    float distance = 0;
    for (int i = 0; i < DIMENSIONS; i++) {
...
__global__ void shiftFunction(float *Ykplus1, float *Yk, float *X, float e) {
    float s = 1;
...
int main(int argc, char **argv) {
    ...
    // Launch kernel
    shiftFunction<<<gridSize, blockSize>>>(deviceArrayYkplus1, deviceArrayYk, deviceArrayStatic, e);
